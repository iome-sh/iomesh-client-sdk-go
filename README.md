...
> **Status:** public OSS **v0.15.x** (pre-1.0). Memory M2/M3 + multi-tenant headers + dual-write/metering + Health/Ready/WaitReady + catalog plane + EvaluatePolicy + QueryContext + ConnectionStatus + ListStreams/GetStream/DeleteStream + CreateStream/EnsureStream `*StreamInfo` + FormatStreams/FormatStreamDetail aligned with [iomesh-tui](https://github.com/iome-sh/iomesh-tui).  
...
```go
fmt.Println(iomeshclient.Version) // e.g. "0.15.0"
// All requests send: User-Agent: iomesh-client-sdk-go/0.15.0
